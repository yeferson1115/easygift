...
<section class="section-agents mt-7">
    <div class="container">
        <div class="row mt-5">
            <h2 class="titlecart mt-3 mb-5 title-checkout"><a href="/" class="btn-return previus-checkout" ><i class="fa-solid fa-angle-left"></i></a>Carrito</h2>
            <div class="col-md-8 mt-5">
                <h4 class="title-products-cart mb-5">Productos</h4>
                ...
                @if (count(Cart::session('secondary')->getContent()))
                    ...
                    @foreach (Cart::session('secondary')->getContent() as $item)
                        ...
                        <div class="row mt-3">
                            <div class="col-4">
                                <img class="image-cart" src="https://kanbai.co/images/products/thumbnail/{{$item->attributes->urlfoto}}" />
                            </div>
                                    ...
                                    <div class="col-6">
                                        <form method="POST" action="">
                                            <div class="form-group">
                                                ...
                                                <button type="submit" data-id="{{ $item->id }}" data-token="{{ csrf_token() }}" data-attr="{{ route('carrito.remove') }}" class="delete-checkout  remove-item" ><i class="fa-regular fa-trash-can"></i></button>
                                            </div>
                                        </form>  
                                    </div>
                                </div>
            ...
            <div class="col-md-4 mt-3 resume-mobile1">
                @if (count(Cart::session('secondary')->getContent()))
                    <div class="resume">
                        ...
                        <div class="mt-4">
                            @php
                            $message .= 'Total: $' . number_format(Cart::session('secondary')->getTotal() + $totalextras+$totalenvio, 0, 0, '.');
                            @endphp
...
<script>
    // Similar to the original script for managing quantities and removals
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        Swal.fire('Muy bien!', 'Item eliminado correctamente', 'success').then(() => {
                            location.reload();
                        });
                    },
                    error: function (data) {
