...
@section('content')
<section class="row calendar-page">
                        ...
                        @foreach($selectedProjects as $project)
                            ...
                            <a class="project-card" href="{{ route('pedidosempresa.show', $project->encode_id) }}">
                                <div class="project-card-info">
                                    ...
                                    <small>Pedido #{{ $project->no_project }}</small>
...
<style>
    ...
    .calendar-title { color: #241f7a; font-size: 2.2rem; line-height: 1; margin: 0; font-weight: 800; }
    ...
    .calendar-grid-left { border-right: 1px solid #ececf1; padding-right: 1.5rem; }
    ...
    .calendar-day { min-height: 74px; border-radius: 12px; border: 1px solid transparent; background: #fff; position: relative; font-weight: 700; color: #4e5462; }
    ...
    .calendar-day-number { position: absolute; top: 10px; left: 10px; }
    .calendar-day-count { position: absolute; bottom: 8px; right: 8px; min-width: 22px; height: 22px; border-radius: 999px; background: #dce4ff; color: #3558b5; font-size: .78rem; display: inline-flex; align-items: center; justify-content: center; padding: 0 .38rem; }
    .calendar-day-selected .calendar-day-count { background: #ffffff; color: #2f67d8; }
    .calendar-grid-right { padding-left: .2rem; }
    .selected-date-title { color: #62718b; margin-bottom: 1rem; font-weight: 800; text-transform: capitalize; }
    ...
    .project-card strong { display: block; font-size: 1.25rem; color: #262c88; line-height: 1.1; }
    ...
    .empty-state { color: #8f95a5; font-weight: 600; }
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const projectsByDate = @json(
            $projects->groupBy(function ($project) {
            ...
            projectsContainer.innerHTML = projects.map((project) => `
                <a class="project-card" href="${project.url}">
                    <div class="project-card-info">
                        ...
                        <small>Pedido #${project.order || '-'}</small>
                        <small>${project.customer || ''}</small>
                    </div>
                    <span class="project-card-btn">Ver</span>
                </a>
